...
function convertToOpenApi3Point0(schema) {
  return walkJsonSchema(schema, (s) => {
    if (Object.keys(s).length === 1 && s.anyOf) {
      ...
      if (s.anyOf.length === 1) {
        return __spreadProps$1(__spreadValues$2({}, s.anyOf[0]), {
          nullable: true
        });
      }
      return {
        anyOf: s.anyOf.map((subSchema) => __spreadProps$1(__spreadValues$2({}, subSchema), {
          nullable: true
        }))
      };
    }
...
function openApiMetadataFactory({
  ...
  const _a = generatedJsonSchema, { properties = {}, $schema = null } = _a, jsonSchema = __objRest(_a, ["properties", "$schema"]);
  if (!isSchemaWithProperties(generatedJsonSchema)) {
    return {
      [PREFIX]: __spreadValues$1({
        type: ""
      }, jsonSchema)
    };
  }
  ...
  for (let [propertyKey, propertySchema] of Object.entries(properties || {})) {
    const newPropertySchema = __spreadValues$1({
      type: ""
    }, propertySchema);
...
function isSchemaWithProperties(jsonSchema) {
  ...
  intercept(context, next) {
    ...
    return next.handle().pipe(
      rxjs.map((res) => {
        if (!responseSchema)
          return res;
        if (res instanceof common.StreamableFile)
          return res;
...
function fixParameter(parameterInput, version) {
  const parameter = deepmerge__default["default"]({}, parameterInput);
