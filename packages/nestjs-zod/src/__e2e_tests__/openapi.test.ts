...
test('output schemas with named sub-schemas', async () => {
  ...
  expect(get(doc, 'components.schemas.BookDto.properties.author.$ref')).toEqual(
    '#/components/schemas/Author908908290384',
  )
...
test('properly adds sub-schemas for array schemas', async () => {
  ...
  expect(get(doc, 'components.schemas.BookDto.items.$ref')).toEqual(
    '#/components/schemas/Author3459835601',
  )
})
...
describe('issue#187', () => {
  ...
  test('correctly reuses schema when used directly as an Output DTO and when used as a sub-schema', async () => {
    class Thing2 extends createZodDto(
      z
        .object({ nameObj: z.object({ name: z.string() }) })
        .meta({ id: 'Thing2' }),
    ) {}
