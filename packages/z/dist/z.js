...
function createMinMaxMapper(valueType, map) {
  return (issue) => {
    if (issue.code !== v3.ZodIssueCode.too_small && issue.code !== v3.ZodIssueCode.too_big) {
      return { matched: false };
    }
...
const extendedErrorMap = (issue, context) => {
  ...
  return v3.defaultErrorMap(issue, context);
};
...
const literal = v3.z.union([v3.z.string(), v3.z.number(), v3.z.boolean()]);
const DEFAULT_MESSAGE = "Expected value to be a JSON-serializable";
const json = (message = DEFAULT_MESSAGE) => {
  const schema = v3.z.lazy(
    () => v3.z.union([literal, v3.z.array(schema), v3.z.record(schema)], {
...
function addIssueToContextExtended(context, issueData) {
  v3.addIssueToContext(context, issueData);
}
...
const _ZodDateString = class extends v3.ZodType {
  _parse(input) {
    const parsedType = this._getType(input);
    const context = this._getOrReturnCtx(input);
    if (parsedType !== v3.ZodParsedType.string) {
      addIssueToContextExtended(context, {
        code: v3.ZodIssueCode.invalid_type,
        expected: v3.ZodParsedType.string,
    ...
    const status = new v3.ParseStatus();
    for (const check of this._def.checks) {
      if (check.kind === "format") {
        ...
        if (valid)
          continue;
        addIssueToContextExtended(context, {
          code: v3.ZodIssueCode.too_small,
...
const _ZodPassword = class extends v3.ZodType {
  _parse(input) {
    const parsedType = this._getType(input);
    const context = this._getOrReturnCtx(input);
    if (parsedType !== v3.ZodParsedType.string) {
